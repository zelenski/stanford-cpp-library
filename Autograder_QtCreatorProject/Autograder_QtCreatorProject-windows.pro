...
#  Used ONLY by marty Stepp for library development on Windows,
#  to get around the limitation that Windows doesn't support symbolic links.
#  This file is NOT to be used as a project template for actual student work.
#  Use the normal stanfordcpplib.pro as that template, even on Windows systems.)
#
...
SOURCES += $$PWD/../StanfordCPPLib/stacktrace/*.cpp
exists($$PWD/src/*.cpp) {
    SOURCES += $$PWD/src/*.cpp
}
...
exists($$PWD/../autograder/*.cpp) {
    # include the various autograder source code and libraries in the build process
    SOURCES += $$PWD/../autograder/*.cpp
    HEADERS += $$PWD/../autograder/*.h
    INCLUDEPATH += $$PWD/../StanfordCPPLib/private/
    INCLUDEPATH += $$PWD/../autograder/
