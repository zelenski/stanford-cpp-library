...
int NOT() {
   ...
   // cout << PriorityQueue_string << endl;
   // cout << Grid_string << endl;
	return 0;
}

template <typename Collection> void readList(Collection& coll)
{
    string s = "{\"red\", \"green\", \"blue\", \"yellow\", \"\", \"blue\"}";
    istringstream in(s);
    if (in >> coll) {
        cout << coll << endl;
    } else {
        cout << " Unable to parse: " << s << endl;
    }
}

template <typename Collection> void readPairs(Collection& coll)
{
    string s = "{0:\"bananas\", 1:\"ice cream\", 2:\"nachos\", 2:\"supernachos\"}";
    istringstream in(s);
    if (in >> coll) {
        cout << coll << endl;
    } else {
...
template <typename Collection> void readGroups(Collection& coll)
{
    string s = "{{\"red\", \"red\", \"red\"}, {\"apple\", \"orange\", \"pear\"}, {\"red\", \"blue\", \"green\"}}";
    istringstream in(s);
    if (in >> coll) {
        cout << coll << endl;
    } else {
