...
public class IntField extends JTextField {
		...
		if (i < minValue || i > maxValue) {
			...
			if (minValue != 0x80000000) {
				if (maxValue != 0x7fffffff)
					s2 = (new StringBuilder()).append(s2).append(" between ").append(minValue)
							.append(" and ").append(maxValue).toString();
				else
					s2 = (new StringBuilder()).append(s2).append(" greater than ").append(minValue)
							.toString();
			} else if (maxValue != 0x7fffffff)
				s2 = (new StringBuilder()).append(s2).append(" less than ").append(maxValue)
						.toString();
			if (dialog == null)
	...
	private String defaultFormat(int i) {
		...
		if (fontmetrics.stringWidth(s) > j)
			for (s = ""; fontmetrics.stringWidth((new StringBuilder()).append(s).append("#")
					.toString()) <= j; s = (new StringBuilder()).append(s).append("#").toString())
				;
		return s;
	}
