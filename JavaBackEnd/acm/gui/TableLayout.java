...
public class TableLayout implements LayoutManager2, Serializable {
	...
	public void addLayoutComponent(Component component, Object obj) {
		if (obj == null) {
			...
			OptionTable optiontable = new OptionTable(((String) obj).toLowerCase(),
					TableConstraints.LEGAL_KEYS);
	...
	public String toString() {
		...
		s = (new StringBuilder()).append(s).append("[rows=").append(nRows).append(",columns=")
	...
	protected TableConstraints lookupConstraints(Component component,
			Container container) {
	...
	private boolean isScrollbar(Component component) {
		return (component instanceof Scrollbar) || (component instanceof JScrollBar)
	...
	private synchronized Dimension lockedProcessLayout(Container container, int i) {
		...
		for (int i1 = 0; i1 < l; i1++) {
			...
			if (j2 == 0) {
				...
				j2 += 2 * tableconstraints.ipadx + tableconstraints.insets.left
						+ tableconstraints.insets.right;
			}
			if (k2 == 0) {
				if (i == 1 || useStrictGridBagModel)
				...
				k2 += 2 * tableconstraints.ipady + tableconstraints.insets.top
						+ tableconstraints.insets.bottom;
			}
		...
		for (int l2 = 0; l2 < k; l2++) {
			j1 += ai1[l2] + hGap;
			ad[l2] = getDoubleProperty((new StringBuilder()).append("weightx").append(l2)
					.toString());
			d += ad[l2];
		}
		...
		for (int i3 = 0; i3 < j; i3++) {
			k1 += ai[i3] + vGap;
			ad1[i3] = getDoubleProperty((new StringBuilder()).append("weighty").append(i3)
					.toString());
			d1 += ad1[i3];
		}
		...
		if (i == 0) {
			...
			for (int i6 = 0; i6 < l; i6++) {
				...
				Dimension dimension2 = useStrictGridBagModel ? getMinimumComponentSize(component1)
						: getPreferredComponentSize(component1);
				Rectangle rectangle = computeCellBounds(new Rectangle(i7, j7, k7, i8), dimension2,
						tableconstraints1);
	...
	private void computeLayoutTable(Container container) {
		...
		for (int i1 = 0; i1 < l; i1++) {
			...
			boolean flag1 = flag;
			setMaxProperty((new StringBuilder()).append("width").append(j).toString(),
					tableconstraints.width);
			setMaxProperty((new StringBuilder()).append("height").append(i).toString(),
					tableconstraints.height);
			setMaxProperty((new StringBuilder()).append("weightx").append(j).toString(),
					tableconstraints.weightx);
			setMaxProperty((new StringBuilder()).append("weighty").append(i).toString(),
					tableconstraints.weighty);
			int k1 = 1;
			...
			case -1:
				if (k <= 0)
					throw new ErrorException(
							"TableLayout: Illegal to use gridwidth=RELATIVE in first row");
				k1 = k - j - 1;
				flag = true;
				break;
	...
	private Rectangle computeCellBounds(Rectangle rectangle, Dimension dimension,
