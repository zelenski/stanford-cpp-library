...
public class GCompound extends GObject implements GContainer {
	...
	public Object clone() {
		...
		for (int i = 0; i < contents.getElementCount(); i++) {
			...
			System.err.println((new StringBuilder()).append("clone    = ")
					.append(gcompound.contents.getElement(i).hashCode()).toString());
		}
	...
	public GRectangle getBounds() {
		...
		Object obj = new java.awt.geom.Rectangle2D.Double(grectangle.getX(), grectangle.getY(),
				grectangle.getWidth(), grectangle.getHeight());
		...
		return new GRectangle(getX() + rectangle.getX(), getY() + rectangle.getY(),
	...
	protected void fireMouseListeners(MouseEvent mouseevent) {
		...
		GPoint gpoint = new GPoint((double) mouseevent.getX() - getX(), (double) mouseevent.getY()
				- getY());
