...
public abstract class GObject implements Cloneable, Serializable, GScalable {
	...
	public void sendToFront() {
		if (compoundParent != null)
			...
			try {
				...
				Class<?> aclass[] = { Class.forName("acm.graphics.GObject") };
				Object aobj[] = { this };
	...
	public Color getColor() {
		...
		for (gobject = this; gobject.objectColor == null;) {
			...
			else if (gcontainer instanceof Component)
	...
	public String toString() {
		...
		return (new StringBuilder()).append(s).append("[").append(paramString()).append("]")
				.toString();
	}
	...
	protected void start(String as[]) {
		try {
			...
			Class<?> aclass[] = { class2, as.getClass() };
			Object aobj[] = { this, as };
	...
	protected String paramString() {
		...
		if (this instanceof GResizable) {
			GRectangle grectangle = getBounds();
			s = (new StringBuilder()).append(s).append("bounds=(").append(grectangle.getX())
					.append(", ").append(grectangle.getY()).append(", ")
					.append(grectangle.getWidth()).append(", ").append(grectangle.getHeight())
					.append(")").toString();
		} else {
			GPoint gpoint = getLocation();
			s = (new StringBuilder()).append(s).append("location=(").append(gpoint.getX())
					.append(", ").append(gpoint.getY()).append(")").toString();
		}
		if (objectColor != null)
			s = (new StringBuilder()).append(s).append(", color=").append(colorName(objectColor))
					.toString();
		if (this instanceof GFillable) {
			s = (new StringBuilder()).append(s).append(", filled=")
					.append(((GFillable) this).isFilled()).toString();
			Color color = ((GFillable) this).getFillColor();
			if (color != null && color != objectColor)
				s = (new StringBuilder()).append(s).append(", fillColor=").append(colorName(color))
						.toString();
		}
		return s;
	}
	...
	protected static String colorName(Color color) {
		...
		if (color.equals(Color.YELLOW)) {
			...
			return (new StringBuilder()).append("0x").append(Integer.toString(i, 16).toUpperCase())
					.toString();
		}
	}
	...
	protected Component getComponent() {
		...
		for (gcontainer = getParent(); gcontainer instanceof GObject; gcontainer = ((GObject) gcontainer)
				.getParent())
			;
