...
public class GImage extends GObject implements GResizable {
	...
	protected void paint2d(Graphics2D graphics2d) {
		...
		if (image != null && component != null) {
			...
			if (color == null)
				graphics2d.drawImage(image, 0, 0, GMath.round(width), GMath.round(height),
						component);
			else
				graphics2d.drawImage(image, 0, 0, GMath.round(width), GMath.round(height), color,
						component);
		}
	}
	...
	public final void setBounds(GRectangle grectangle) {
		if (getMatrix() != null) {
			...
			setBounds(grectangle.getX(), grectangle.getY(), grectangle.getWidth(),
