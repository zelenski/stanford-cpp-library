...
class VertexList implements Serializable {
	...
	public synchronized GRectangle getBounds(AffineTransform affinetransform) {
		...
		for (int j = 0; j < vertices.size(); j++) {
			...
			java.awt.geom.Point2D.Double double1 = new java.awt.geom.Point2D.Double(gpoint.getX(),
					gpoint.getY());
	...
	public synchronized boolean contains(double d, double d1, AffineTransform affinetransform) {
		...
		for (int j = 0; j < i; j++) {
			...
			java.awt.geom.Point2D.Double double2 = new java.awt.geom.Point2D.Double(gpoint.getX(),
					gpoint.getY());
			if (affinetransform != null)
				affinetransform.transform(double2, double2);
			if ((double1.getY() < d1 && double2.getY() >= d1 || double2.getY() < d1
					&& double1.getY() >= d1)
					&& double1.getX() + ((d1 - double1.getY()) / (double2.getY() - double1.getY()))
							* (double2.getX() - double1.getX()) < d)
				flag = !flag;
		}
