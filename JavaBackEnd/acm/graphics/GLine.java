...
public class GLine extends GObject {
	...
	protected void paint2d(Graphics2D graphics2d) {
		java.awt.geom.Line2D.Double double1 = new java.awt.geom.Line2D.Double(0.0D, 0.0D,
				GMath.round(dx), GMath.round(dy));
	...
	public boolean contains(double d, double d1) {
		...
		if ((float) d2 - (float) d4 == 0.0F && (float) d3 - (float) d5 == 0.0F) {
			...
			double d7 = ((d - d2) * (d4 - d2) + (d1 - d3) * (d5 - d3))
					/ distanceSquared(d2, d3, d4, d5);
	...
	public String paramString() {
		...
		String s1 = (new StringBuilder()).append("start=(").append(gpoint.getX()).append(", ")
				.append(gpoint.getY()).append(")").toString();
		gpoint = getEndPoint();
		s1 = (new StringBuilder()).append(s1).append(", end=(").append(gpoint.getX()).append(", ")
				.append(gpoint.getY()).append(")").toString();
