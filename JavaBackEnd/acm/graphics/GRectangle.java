...
public class GRectangle implements Serializable {
	...
	public GRectangle intersection(GRectangle grectangle) {
		...
		double d3 = Math
				.min(grectangle1.yc + grectangle1.height, grectangle.yc + grectangle.height);
	...
	public GRectangle union(GRectangle grectangle) {
		...
		if (grectangle.isEmpty()) {
			...
			double d2 = Math.max(grectangle1.xc + grectangle1.width, grectangle.xc
					+ grectangle.width);
			double d3 = Math.max(grectangle1.yc + grectangle1.height, grectangle.yc
					+ grectangle.height);
			return new GRectangle(d, d1, d2 - d, d3 - d1);
		}
	}
	...
	public Rectangle toRectangle() {
		return new Rectangle(GMath.round(xc), GMath.round(yc), GMath.round(width),
				GMath.round(height));
	}
	...
	public String toString() {
		return (new StringBuilder()).append("[").append((float) xc).append(", ").append((float) yc)
				.append(", ").append((float) width).append("x").append((float) height).append("]")
				.toString();
	}
