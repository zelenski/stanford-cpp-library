...
public class GDimension implements Serializable {
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof GDimension)) {
			...
			return (float) width == (float) gdimension.width
					&& (float) height == (float) gdimension.height;
		}
	}

	public String toString() {
		return (new StringBuilder()).append("(").append((float) width).append("x")
				.append((float) height).append(")").toString();
	}
