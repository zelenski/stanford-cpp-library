...
public class IOConsole extends Container implements IOModel {
	...
	public int readInt(String s, int i, int j) {
		while (true) {
			...
			String s1;
			try {
				s1 = (new StringBuilder()).append("Value is outside the range [").append(i)
	...
	public double readDouble(String s, double d, double d1) {
		while (true) {
			...
			String s1;
			try {
				s1 = (new StringBuilder()).append("Value is outside the range [").append(d)
						.append(":").append(d1).append("]").toString();
			} catch (NumberFormatException numberformatexception) {
				s1 = "Illegal numeric format";
			}
			showErrorMessage(s1);
...
_L1:
	...
	private static final String CONSOLE_MENU_ACTIONS[] = { "Save", "Save As", "Print Console",
			"Script", "Cut", "Copy", "Paste", "Select All" };
