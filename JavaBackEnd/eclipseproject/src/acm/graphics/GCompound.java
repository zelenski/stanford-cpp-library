...
public class GCompound extends GObject implements GContainer {
	...
	public Object clone() {
		...
		for (int i = 0; i < contents.getElementCount(); i++) {
			System.err.println((new StringBuilder()).append("original = ")
					.append(contents.getElement(i).hashCode()).toString());
			System.err.println((new StringBuilder()).append("clone    = ")
					.append(gcompound.contents.getElement(i).hashCode())
					.toString());
		}
	...
	public GRectangle getBounds() {
		...
		Object obj = new java.awt.geom.Rectangle2D.Double(grectangle.getX(),
				grectangle.getY(), grectangle.getWidth(),
	...
	public GPoint getCanvasPoint(double d, double d1) {
		GCompound gcompound;
		for (Object obj = this; obj instanceof GCompound; obj = gcompound
				.getParent()) {
	...
	protected void fireMouseListeners(MouseEvent mouseevent) {
		...
		GPoint gpoint = new GPoint((double) mouseevent.getX() - getX(),
				(double) mouseevent.getY() - getY());
		GObject gobject = getElementAt(gpoint);
		MouseEvent mouseevent1 = null;
		if (gobject != lastObject) {
			if (lastObject != null) {
				mouseevent1 = GCanvas.createMouseEvent(lastObject, 505,
						mouseevent);
				lastObject.fireMouseListeners(mouseevent1);
			}
			if (gobject != null) {
				mouseevent1 = GCanvas
						.createMouseEvent(gobject, 504, mouseevent);
